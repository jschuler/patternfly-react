          ...
          switch (raw) {
            ...
            default:
              return {
                name,
                value:
                  'unrecognizedType ' + name + ' ' + value + ', consider reporting error to react-jest-test-generator.'
              };
          }
      }
  }
...
module.exports = class extends Generator {
  ...
  writing() {
    const filePaths = read(this.props.COMPONENTS_PATH).filter(
      filename => filename.endsWith('.tsx') && filename.indexOf('.test.') < 0
    );
    ...
    for (let i = 0; i < metadata.length; i += 1) {
      ...
      const testPath = path.resolve(
        ...
        path.join('..', '__tests__', compMetaData.filename + '.test.js')
      );
      ...
      try {
        ...
        const formattedTestCode = this.options.prettify
          ? prettier.format(generatedTestCode, {
              singleQuote: true,
              trailingComma: 'all'
            })
          : generatedTestCode;
